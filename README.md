...
- [ ] **集成 ai-eyes 项目**（用户已有 github.com/kttx007/ai-eyes）
- [ ] **使用 MiniMax-M3 原生多模态**直接看图（不是外部 VLM）
- [ ] **无限画布**能力（Photoshop 风格图层合成）
- [ ] **真正调研 + 拆解爆款**（Codex 旧报告有，但没延续）
- [ ] **第一版图全部重做**（VLM 平均 3/10，已废弃）
- [ ] **多产品**（只有 C03 一个实例）
- [ ] **审计重写**（当前审计是摆设）
